...
\begin{frame}[t]\frametitle{\insertsection}
  ...
  \begin{description}
    ...
    \item[Two-sided Conformal Region] \hfill\\
      \textit{upper} $A(Z_{-i}, z_i) = \hat{r}_i(y_{n+1}; Z_{:n}, x_{n+1})$ and \hfill\\
...
\begin{frame}[c]\frametitle{\insertsection}
  \framesubtitle{\insertsubsection}
  Same constructon, less endpoints: each non-empty $S_i$ is \begin{itemize}
...
\begin{frame}[c]\frametitle{\insertsection}
  ...
  \begin{block}{Conjecture \# 1}
    ...
    then all $S_i$ for upper and lower NCM are of the form $(-\infty, a_i]$ and
    $[a_i, +\infty)$ respectively.
  \end{block}
\end{frame}
...
\begin{frame}[c]\frametitle{\insertsection}
  \framesubtitle{\insertsubsection}
  By design, conformal prediction algorithm iss oblivious to the nature of
