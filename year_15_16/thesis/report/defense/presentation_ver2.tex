...
\begin{frame}[c]\frametitle{\insertsection}
  ...
  \begin{block}{So far:}
  \begin{itemize}
    \item an effcient ploynomial-time algorithm for construction of conformal
    confidence regions;
...
\begin{frame}[c]\frametitle{\insertsection}
  ...
  \vspace{\baselineskip}
  \begin{block}{Maximum a posteriori prediction}
...
\begin{frame}[c]\frametitle{\insertsection}
  ...
  \begin{figure}%[t, width=0.45\textwidth]
    ...
    \caption{Example Bayesian and conformal (RRCM) confidence regions.
      \textit{Upward} triangles indicate the $5\%$ sample quantile across the whole
      test sample of confidence regions' widths, whereas \textit{downward} triangles
      indicate the maximal width. The median width is drawn with a slightly thicker
      line.}
...
\begin{frame}[c]\frametitle{\insertsection}
  \framesubtitle{\insertsubsection}
  Conformal prediction algorithm is by design oblivious to the nature of
...
\begin{frame}[t]\frametitle{\insertsection}
  ...
  \begin{block}{Conformal Bandwidth Selection}
    ...
    \end{itemize}
    \vspace{\baselineskip}
    Main challenge: move from local to global bandwidth;
  \end{block}
\end{frame}
