...
or online filtering of potentially interesting events, it is necessary not only
to make optimal predictions with respect to some loss, but also to be able to quantify
the degree of confidence in the generated prediction. Standard regression methods,
however, do not readily provide the user with such performance metric, and additional
probabilistic assumptions regarding the observation model (eq.~\ref{eq:signal_model})
...
performance when Gaussianity assumptions are satisfied.
...
interpretation of the KRR.

The report is structured as follows: in sec.~\ref{sec:kernel_ridge_regression} we
provide an brief overview of the machinery behind the kernel ridge regression, in
...
\subsection{Kernel CRR confidence predictor} % (fold)
...
hyper-parameters: \begin{enumerate}
  ...
  \item the precision of the isotropic Gaussain kernel $\theta$ is picked from
  $\{10, 10^2, 10^3\}$, since the input domain is the unit cube, for which
  $\sup_{x,x'\in[0,1]^d}\|x-x'\|^2 = d$;
...
fig.~\ref{fig:nongauss_2d_f2_profile} and~\ref{fig:nongauss_2d_f5_profile}, observations
of which are contaminated by moderate Gaussian noise.
...
Table~\ref{tab:gaussian_2d_cov_gpr} shows the error rates ($y\notin\Bcal$) of the GPR
confidence intervals. Columns 1 and 4 show that the regions are approximately valid
in the cases when the KRR parameters are set to the true values used to generate
the data and with larger train sample the coverage improves. As in the $1$-d case
the intervals are more conservative for the case of low noise in the data $\gamma=10^{-6}$
and high regularization $\lambda=10^{-1}$. Table~\ref{tab:gaussian_2d_cov_gpr_neq}
show that in current setting, the validity of the GPR confidence intervals is sensitive
to misspecification of the kernel precision.

\begin{table}
  \centering
  \caption{Goodness of fit of the KRR of the simulated $2$-d Gaussian process.}
...
\centering
  ...
  simulated $2$-d Gaussian process.}
...
\begin{table}
  \centering
  \caption{The empirical error rate ($\%$) of the GPR confidence interval for the
  simulated $2$-d Gaussian process for misspecified $\theta$.}
...
$A = \{1\%, 5\%, 10\%, 25\%\}$, $(x^*_j, y^*_j)_{j=1}^m$ -- the test sample, and
...
\begin{table}
  ...
  of the empirical error rate from the theoretical significance level of the GPR
  confidence interval for the simulated $2$-d Gaussian process.}
...
We proceed to the non-Gaussian case. The test functions we use for this are plotted
in fig.~\ref{fig:nonggauus_2d_f2_profile} and~\ref{fig:nonggauus_2d_f5_profile}.
...
\begin{table}
  ...
  $n$ & $\theta$ &          &          &          &          &          &          &          &          \\
  \midrule
  $150$  & $10^1$ &     0.45 &     0.43 &     4.67 &     0.65 &     7.15 &     0.61 &     6.19 &     0.68 \\
       & $10^2$ &     1.75 &     1.83 &     1.81 &     1.70 &     1.35 &     1.49 &     1.42 &     1.39 \\
       & $10^3$ &     3.21 &     3.24 &     2.92 &     2.93 &     2.52 &     2.51 &     2.40 &     2.41 \\
       & $\hat{\theta}_\text{ML}$ &     0.37 &     0.44 &     1.67 &     0.72 &     1.04 &     0.67 &     1.28 &     0.70 \\
  \midrule
  $1500$ & $10^1$ &     0.06 &     0.19 &     0.56 &     0.49 &     0.39 &     0.43 &     0.51 &     0.51 \\
       & $10^2$ &     0.05 &     0.14 &     5.57 &     0.54 &     2.03 &     0.32 &     4.16 &     0.47 \\
       & $10^3$ &     1.66 &     1.78 &     1.68 &     1.63 &     1.33 &     1.38 &     1.38 &     1.36 \\
       & $\hat{\theta}_\text{ML}$ &     0.02 &     0.09 &     1.97 &     0.48 &     0.67 &     0.32 &     1.25 &     0.46 \\
  \bottomrule
  \end{tabular}
\end{table}
...
In the non-Gaussian experiments we found empirical evidence suggest that the GPR confidence
...
seem to perform not worse than the confidence regions of the Gaussian Process regression,
