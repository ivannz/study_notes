...
$\|K_{-n}(x_n)\|^2\to 0$. Since the RBF kernel is bounded, the value $m_n$ (eq.~
\ref{eq:krr_leverage}) is a bounded function of $x_n$, which, in turn, implies that
eventually all RRCM regions $S_i$ assume the form of closed intervals $[-|q_i|, |q_i|]$,
where $q_i = m_n (e_i'Q_{-n}y_{-n}) + o(\|x_n\|^2)$, $i \neq n$. The same is true
for the CRR procedure. Therefore, the conformal procedure essentially reverts to
a constant-size confidence region, determined by the  $n^{-1}\lfloor n(1-\alpha)\rfloor$-th
...
\begin{figure}%[t, width=0.5\textwidth]
  ...
  \end{subfigure}
  \caption{Sample Bayesian and conformal confidence bands for the ``Heaviside''
...
\begin{figure}%[t, width=0.5\textwidth]
  ...
  \end{subfigure}
  \caption{Sample Bayesian and conformal confidence bands for the ``f6''
  function (train sample size $n=50$): \textit{left} -- GPR, \textit{middle} -- CRR,
  and \textit{right} -- RRCM.}
...
by the KRR as demonstrated by the MSE-var ratio. At the same time conformal procedures,
e.g. RRCM, show no significant departures from claimed validity.

This might seem strange, since in the gaussian case we argued that conformal confidence
regions in the low noise case should be wider and thus more conservative. In the non-gaussain
case the KRR does not fit the test functions quite as well, due to the fact that

% This might be the kernel effect: the fact that RBF kernel has polyomial 

whereas in the former case the coverage rate over all
significance levels, in the latter 

As for the Gaussian confidence regions, the converage in their case is not 
First of all, all confidence regions exhibit the same expansion near the boundaries
of the interval to which the train data is confined.


% for the Heaviside step function (fig.~\ref{fig:nongauss_1d_heaviside}), and the ``f6''
% function (fig.~\ref{fig:nongauss_1d_f6}).

 % In this setting the GPR confidence regions 
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage dynamics for the the pressure2 step function.}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage dynamics for the the pressure2 step function(c.f. fig.~\ref{fig:pressure2_1d_low_noise}).}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage dynamics and MSE-to-var ratio for the an ``f6'' function.}
