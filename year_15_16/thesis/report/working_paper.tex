...
test prediction $\hat{y}_x$ to the test variance of the observed $y_x$, in order
to monitor the quality of the fit.

%% Correct \theta, correct high \lambda and \gamma
...
in favor of the conjectured asymptotic conservative validity of the conformal
...
%% Correct \theta, correct low \lambda and \gamma
...
%% Correct \theta, incorrect \lambda and \gamma
...
%% incorrect \theta, correct high \lambda and \gamma
...
when the kernel in KRR is imprecise ($\theta << \theta_0$), the resulting KRR estimate
...
%% incorrect \theta, correct low \lambda and \gamma

%% incorrect \theta, incorrect \lambda and \gamma

%% non-gaussian correct high \lambda and \gamma
%% non-gaussian correct low \lambda and \gamma
%% non-gaussian incorrect \lambda and \gamma
...
We check if the procedure are equivalent in the $1$-d setting for different data
generating processes and kernel ridge regression hyper-parameters. We begin by
considering the low noise experiments, which simulate the interpolation setting.
...
One important observation is that conformal procedures (eq.\ref{eq:conf_pred_set})
...
\ref{fig:gaussian_1d_low_noise}). The randomness in the convergence plots are entirely
due to the effects of random choice of the train input data, against which the test
is checked for conformity. Within each column the confidence regions were computed
under exactly the same conditions: for each hyper-parameter setting (excluding $\gamma$)
we drew $L$ random samples from some fixed realization of a Gaussian process (with
kernel precision $\theta_0$), compute the ML estimate of $\theta$, if necessary, and
then constructed the confidence intervals.
...
adapts to the noise level not through the regularization $\lambda$, bu through
...
In the non-gaussian setting we consider two alternatives

Nevertheless, in the noiseless case the necessity of confidence measures of predictions
seems questionable.
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  ``CRR'', ``CRR-loo'', and the last depicting ``mse-var'' ratio.}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Region size dynamics in the fully gaussian low-noise case $\gamma=10^{-6}$
  for different $(\theta, \lambda)$ (see description in fig.~\ref{fig:gaussian_1d_low_noise}).}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Region size dynamic (see description in fig.~\ref{fig:gaussian_1d_low_noise_arb})
  for arbitrary $\theta$ in the fully gaussian case.}
...
% Sample confidence intervals in the current non-Gaussian setting are provided in fig.~\ref{fig:non_gauss_1d_prof_conf}.
\begin{figure}%[b, width=0.5\textwidth]
  \centering
  \begin{subfigure}[b]{0.5\linewidth}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \begin{subfigure}[b]{0.25\linewidth}
    ...
    \caption{} \label{fig:nongauss_1d_low_noise_c1}
  \end{subfigure}%
  \begin{subfigure}[b]{0.25\linewidth}
    \includegraphics[width=0.95\linewidth]{images/output_pdf/exp_1d/pressure2/1e-06_0.1/pressure2_1e-06_0,1_100_ratio.pdf}
    \caption{} \label{fig:nongauss_1d_low_noise_c2}
  \end{subfigure}%
  \begin{subfigure}[b]{0.25\linewidth}
    \includegraphics[width=0.95\linewidth]{images/output_pdf/exp_1d/pressure2/1e-06_1e-06/pressure2_1e-06_1e-06_auto_ratio.pdf}
    \caption{} \label{fig:nongauss_1d_low_noise_c3}
  \end{subfigure}%
  \begin{subfigure}[b]{0.25\linewidth}
    \includegraphics[width=0.95\linewidth]{images/output_pdf/exp_1d/pressure2/1e-06_0.1/pressure2_1e-06_0,1_auto_ratio.pdf}
    \caption{} \label{fig:nongauss_1d_low_noise_c4}
  \end{subfigure}%
  \caption{Coverage dynamics (rows 1 and 2) and MSE-to-var ratio (row 3) for a non-gaussian
  function (``pressure2''). Coverage dynamics for all confidence regions (``RRCM'',
  ``RRCM-loo'', ``CRR'', and ``CRR-loo'') is identical.}
  \label{fig:nongauss_1d_low_noise}
\end{figure}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \begin{subfigure}[b]{0.25\linewidth}
    ...
    \caption{} \label{fig:nongauss_1d_low_noise_c4}
  \end{subfigure}%
  \caption{Coverage dynamics and MSE-to-var ratio for the an ``f6'' function (see
  the description of fig.~\ref{fig:nongauss_1d_low_noise}).}
  \label{fig:nongauss_1d_low_noise}
\end{figure}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \begin{subfigure}[b]{0.25\linewidth}
    ...
    \caption{} \label{fig:nongauss_1d_low_noise_c4}
  \end{subfigure}%
  \caption{Coverage dynamics for the the Heaviside step function (c.f. fig.~\ref{fig:nongauss_1d_low_noise}).}
  \label{fig:nongauss_1d_low_noise}
\end{figure}

, since due to
continuity of $A\bigl((X_{-i}, y_{-i}), (x_i, y_i)\bigr)$ as a function of $y_n$,
the ordering of $\eta_i^{\tilde{z}}$ in a non degenerate case (with no ties), remains
unchanged 


$\eta_i^{\tilde{z}} = A(S^{\tilde{z}}_{-i}, S^{\tilde{z}}_i)$
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage rate dynamics in the $\theta=\theta_0$ gaussian case.}
\end{figure}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Width dynamics in the $\theta=\theta_0$ gaussian case.}
\end{figure}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage rate dynamics in the $\theta\neq\theta_0$ gaussian case with ML estimate of $\theta$.}
\end{figure}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Width dynamics in the $\theta\neq\theta_0$ gaussian case with ML estimate of $\theta$.}
\end{figure}

The plots show that no matter the what the parameters are, in the gaussian case all
for procedures show almost identical coverage and width convergence.
