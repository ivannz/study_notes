...
The problem of conformalized ridge regression (CRR) has been studied in \cite{BurVovk2014},
...
Let $\Xcal = \Real^{d\times 1}$ and $\Ycal=\Real$, and recall the RR problem. The
conformal procedure in \cite{BurVovk2014} is constructed from a conformity measure
defined over a sample $Z=(x_i, y_i)_{i=1}^n \in \Zcal$ as:
...
\end{equation}
with $X\in\Real^{n\times d}$ being the overall design matrix of $Z$ and $y\in\Real^{n\times1}$
-- the target vector of the sample $Z$.

In \cite{BurVovk2014} it was shown that for any $\epsilon\in(0,1)$ the confidence
...
mentioned in sec.~\ref{sub:confromalized_ridge_regression}.

Recall that, for any $\epsilon\in(0,1)$ the confidence region $\Gamma^\epsilon$
output by CRR procedure for the NCM in eq.~\ref{eq:crr_ncm} at is equivalent to
the intersection of confidence sets yielded by conformal procedures with NCM scores
given by $\hat{r}_i^z$ (upper) and $-\hat{r}_i^z$ (lower) at significance level
$\frac{\epsilon}{2}$.
