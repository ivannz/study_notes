...
and $\beta$ steps, it is possible to gradually estimate the optimal parameters.
...
maps with respect to the uniform norm $\|\cdot\|_\infty$, see.~\cite{rbf_universal}.
...
by varying the following hyper-parameters: \begin{enumerate}
  ...
  \item the effect of the train sample size $n$: small $n \in \{ 25, 50, 100\}$,
  moderate-to-large $n \in \{200 k\,:\, k=1, \ldots, 8 \}$, -- to study the effects
  the train sample size has on validity and efficiency;
  \item the NCM $A$ for the confidence region: either the RRCM, or the CRR;
  \item the residual $\hat{r}$: either $\hat{r}_{\text{in}}$, or $\hat{r}_{\text{loo}}$;
\end{enumerate}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item draw an independent random sample of size $n$ without replacement from
    the full train set $\tilde{T} = (x, y_x)_{x\in X}$;
    \item center the training sample;
    \item fix an RBF kernel $K$ with the specified precision $\theta$ and fit a
    Gaussian Process regression with $m(x) = 0$ to the train dataset $\tilde{T}$
    using the kernel $\Kcal = \sigma^2( \lambda \delta_{x,x'} + K(x,x') )$, where
    the values $\lambda > 0$ is specified and the amplitude $\sigma^2$ is estimated;
    \item for each $x^* \in X^*$ construct, the Bayesian KRR region, $\Bcal_l^\alpha(x^*)$,
    and the conformal confidence regions, $\Gamma_l^\alpha(x^*; A, \hat{r})$ using the
    specified NCM $A$, the residuals $\hat{r}$, and the kernel $\Kcal$ (using the estimated
    $\sigma^2$);
    \item for the centered test data estimate the coverage rate and the width of
    the convex hull of the region: $p_l(R) = |X^*|^{-1}\sum_{x\in X^*} 1_{y_x\in R_x}$,
