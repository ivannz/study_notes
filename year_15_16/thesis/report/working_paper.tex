...
for the RBF kernel the conformal confidence intervals seem to perform reasonably well
both in terms of validity in a non-Gaussian setting and efficiency in Gaussian setting.
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage dynamics and MSE-to-var ratio for the ``f6'' function.}
...
\begin{figure}%[b, width=0.5\textwidth]
  ...
  \end{subfigure}%
  \caption{Coverage dynamics and MSE-to-var ratio for the ``f6'' function (see
...
procedures over KRR with RBF kernel.
...
intervals constructed with an NCM based on KRR with RBF kernel provide at least conservatively
the required level of validity regardless of the parameters ($\theta$, $\lambda$)
at the core of the non-conformity measure.
