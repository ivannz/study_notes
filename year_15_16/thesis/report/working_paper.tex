...
\paragraph{Anomaly detection based on predictive modeling} % (fold)
...
% paragraph anomaly_detection_based_on_predictive_modeling (end)
...
The proposed test statistic measures, the \textit{maximum mean discrepancy} (MDD),
which measures the RKHS norm of the deviation of the embeddings of the empirical
distributions. This MDD measure can readily be used as a non-conformity measure for
the following anomaly detection problem: given a sample $Z_{:n-1} = (z_i)_{i=1}^{n-1}$
...
could provide for such unsupervised machine learning algorithms, as the LOF (\cite{breunig2000},
described in sec.~\ref{par:anomaly_detection_based_on_distance_similarity} of this report),
Bayesian PCA and mixtures thereof \cite{tippnigbishop1999}, and the kernel PCA \cite{hoffmann2007}.
