...
random_state = np.random.RandomState(seeds_[2])
...
for i_, par_ in enumerate(grid_):
    ...
    while n_replications > 0:
        ...
        X = 2*random_state.uniform(size=(size_, nd)) - 1
    ...
    if len(experiment) >= 25:
        basename_ = os.path.join(BASE_PATH, "exp_nongauss_2d%04d"%(batch_,))
