...
BASE_PATH = mkdirifnot(os.path.join(".", "exp2"))
...
funcs_ = ['f2', 'f5']
func2d_ = {fname_: fn_
           for fname_, fn_ in func2D().iteritems() if fname_ in funcs_}

## Define the grid
grid_ = ParameterGrid(dict(dgp=func2d_.values(),
                           size=[150, 1500,],
                           nugget=[1e-6, 1e-2,],
...
for i_, par_ in enumerate(grid_):
    ...
    ## Draw random train sample
        X_train = random_state.uniform(size=(size_, nd))*2 - 1
...
## Consolidate the simultions
    ...
    if len(experiment) >= 25:
        basename_ = os.path.join(BASE_PATH, "exp2_%04d"%(batch_,))
