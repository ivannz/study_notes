...
BASE_PATH = os.path.join(".", "exp_gauss_2d")
...
grid_ = ParameterGrid(dict(size=[150, 1500,],
                           nugget=true_nugget,
...
XX_train = random_state.uniform(size=(15001, nd))
...
for noise_ in true_nugget:
    ...
    for i_, par_ in enumerate(grid_):
        ...
        if len(experiment) >= 25:
            basename_ = os.path.join(BASE_PATH, "exp_gauss_2d%04d"%(batch_,))
