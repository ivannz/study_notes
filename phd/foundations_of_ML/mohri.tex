...
KKT conditions are: \begin{description}
    \item[Primal] $\forall i=1,\ldota, m$ we must have $y_i(\beta'x_i + \beta_0) \geq 1$;
    \item[Dual] $\forall i=1,\ldota, m$ we must have $\alpha \geq 0$;
    \item[Complementary slackness] $\alpha_i\bigl(1-y_i(\beta'x_i + \beta_0)\bigr) = 0$ for all $i=1,\ldota, m$;
...
given by:
\[ h(x) = \text{sgn}(\sum_{i=1}^m \alpha_i y_i x_i'x + \beta_0_ \,,\]
...
\[ H = \{x\masto x'\beta\,:\,\|\beta\|\leq \Lambda\} \,. \]
Then
...
The generalization. Let $S\subsetq \{x\,:\, \|\phi(x)\|_\Hcal \leq R\}$ and
