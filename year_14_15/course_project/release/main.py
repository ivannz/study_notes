...
## Project modules
## Import the Dietrich and Newsam (1997) fGn synthesizer.
# from processes.fgn_pyfftw import fgn
# from processes.fgn_numpy import fgn
## I used to use dirty hacks to implement dynamic inheritance( BAD!
## No i don't: you should have to change manually the import in
## scripts processes/fbm.py and processes/hermite.py

## Import the derived processes
from processes.fbm import fbm
from processes.hermite import hermite

## Import the randomized Weierstrass function
from processes.weierstrass import weierstrass

## Include the framework for parallel Monte Carlo
from montecarlo import mc_run as montecarlo

## And import the crossing tree constructor
...
## Analyse the sample path
	return delta, path_analyse( T, X, delta, max_levels, max_crossings )
...
def sim_load( file_name, return_durations = False, without_delta = False ) :
## Load the npz file
...
## Remove the extension and split by underscores: the format is (G, D, dttm, P, H, M)
	...
	if not return_durations or 'Wjnp' not in data :
## return the basic results of the simulation : tuple( H, Njn, Djnk, Vjnde )
		if without_delta :
			return float( par[ -2 ] ), data[ 'Njn' ], data[ 'Djnk' ], data[ 'Vjnde' ], tuple( )
		else :
			return float( par[ -2 ] ), data[ 'deltaj' ], data[ 'Njn' ], data[ 'Djnk' ], data[ 'Vjnde' ], tuple( )
	else :
...
## return the results of the simulation : basic plus summary of duration data
		if without_delta :
			...
			return float( par[ -2 ] ), data[ 'deltaj' ], data[ 'Njn' ], data[ 'Djnk' ], data[ 'Vjnde' ], ( Wjnp, Wbarjn, Wstdjn )

## A procedure for saving the results of the Monte-Carlo simulation
def sim_save( filename, result, save_durations = False, **extra ) :
	npz_data = dict( **extra )
	npz_data[ "deltaj" ]  = np.array( [ d for wrk, j, ( d, ( _, _, _, _ ) ) in result ] )
	npz_data[ "Njn" ]     = np.array( [ n for wrk, j, ( _, ( n, _, _, _ ) ) in result ] )
	npz_data[ "Djnk" ]    = np.array( [ d for wrk, j, ( _, ( _, d, _, _ ) ) in result ] )
	npz_data[ "Vjnde" ]   = np.array( [ v for wrk, j, ( _, ( _, _, v, _ ) ) in result ] )
## If requested save the measured parameters of durations as well
	if save_durations :
		npz_data["Wjnp"]    = np.array( [ w for wrk, j, ( _, ( _, _, _, ( w, _, _ ) ) ) in result ] )
		npz_data["Wbarjn"]  = np.array( [ b for wrk, j, ( _, ( _, _, _, ( _, b, _ ) ) ) in result ] )
		npz_data["Wstdjn"]  = np.array( [ s for wrk, j, ( _, ( _, _, _, ( _, _, s ) ) ) in result ] )
...
## The base path of the the results
	basepath = os.path.realpath( "./output/test01/" )
####################################################################################################
	if False :
...
## Initialize the Hermite process generator
					...
					print "Monte-Carlo (%d) for Hermite-%d(2**%d+1-%d, %.4f), %s:" % ( M, D, P, K, H, delta_method, )
					start_dttm = datetime.utcnow( )
					results = montecarlo( generator, mc_kernel,
						processes = 2, debug = False, quiet = False, parallel = True,
						replications = M, delta = delta_method, L = 20, K = 40 )
...
## Initalize the generator
				...
				print "Monte-Carlo (%d) for Weierstrass(2**%d+1, %.4f), %s:" % ( M, P, H, delta_method, )
## Get the current timestamp
				start_dttm = datetime.utcnow( )
## Run the experiment
				results = montecarlo( generator, mc_kernel,
