...
\noindent \textbf{Conjecture}\begin{itemize}
    ...
    for all $m=1,2,\ldots$;
    \item The distribution of excursion patterns $V_k^n$, conditional on the orientation
...
\end{itemize}
The rational behind the scaling in the crossing duration case is given below. By
definition, a the duration of a crossing of grid $\delta 2^n \mathbb{Z}$ (level-
...
Crossing duration also do not seem contradict the hypothesized scaling for $H$-sssi proceses
and the Weierstrass function (see. fig.~\ref{fig:wei_offspring_down_up} - \ref{fig:hrm_4_offspring_down_up}).
...
\label{fig:wei_offspring_down_up}
\end{center}\end{figure}
...
\label{fig:hrm_2_offspring_down_up}
\end{center}\end{figure}
...
\label{fig:hrm_3_offspring_down_up}
\end{center}\end{figure}
...
\label{fig:hrm_4_offspring_down_up}
\end{center}\end{figure}
