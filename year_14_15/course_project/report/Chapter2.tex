...
diffusion on fractal sets \ref{BarlowPerkins88}. It was later reintroduced by Jones
...
took place allows further refinement of crossing times' behaviour: under the assumptions
...
$\delta 2^n \mathbb{Z}$ for $n\geq0$. For example in~\ref{decrouez2013estimation} the
...
regularly spaced but finitely many point in time.
...
of the tree are computed for a given fixed $\delta>0$ in two passes: the first pass
computes passage times on the passed $x_0+\delta \mathbb{Z}$ grid levels, and the
second pass adjusts the passage times and passed levels to eliminate re-crossings
...
trade-off:
\begin{itemize}
    \item the grid with too low value of $\delta$ would make the resolution so fine
...
Brownian motion on a nested fractal (see~\ref{BarlowPerkins88}).
