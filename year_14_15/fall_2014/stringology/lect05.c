// Word tree
// Бор (trie) is a rooted tree with:
// • each word represents a path in the tree (within-tree )
// • each node has children with distinct symbols
// • every path from the root, to a leaf represents an actual word (as a sequence of nodes)

// Trie building procedure is sequentailly adding path suffixes to the tree.
//  or each node has a fixed array indexed by the alphabet: O(1) access time
//  or every node points to its private BST, which locates the suffix sub-tree.

// Task: given \brac{P_i}_{i=1}^k

// KMP algo: shift to the suffix (maximal boundary) of the matched part of the pattern

// Failure links (similar to suffix links): are unique and defined for each node.
// a trie is correctly constructed no duplicate prefixes on different sub-paths.
// u->v has a Failure link if the prefix of U and prefix of V is the longest prefix among
//   patterns equal to the proper suffix of U

// empty suffix -> empty label -> root
// proper suffix -> up 

// A trie with failure links is a finite state machine

// Simultaneous maximal boundary computation

// match(P^1...P^k, T)
// • walk along the trie while reading T 
// • follow the Failure link in failure (compare with B[B[...B[k]]] )

// Output (exit) link u->v if v is the closest pattern from u following the failure links.

// number of matches O(n^2)

// One might also compactify the trie, by coalescing the sub-paths with no forks.

// A suffix tree some TEXT is a compactified trie on the suffixes of TEXT[1:k]+$
