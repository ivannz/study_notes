...
\begin{prop} Let $(P,\leq)$ and $(Q,\leq)$ be posets and $\phi:P\to Q$ be a bijective map. Then the following are equivalent: \begin{enumerate}
...
Let $(P,\leq)$ and $Q\subseteq P$
\begin{description}
	\item[Order Ideal] whenever $x\in Q$ and $y\in P$ with $y\leq x$, we have $y\in Q$
	\item[Order Filter] whenever $x\in Q$ and $y\in P$ with $x\leq y$, we have $y\in Q$
\end{description}
ideals (and filters) are ordered by set inclusion $\subseteq$. Let $\mathcal{O}(P)$ be the set of all order ideals of $P$.

\[Q^\downarrow \defn  \obj{ \induc{ y\in P }\,\exists x\in Q,\, x\leq y }\]
\[Q^\uparrow \defn  \obj{ \induc{ y\in P }\,\exists x\in Q,\, y\leq x }\]

\begin{prop} If $(P,\leq)$ is  a poset and $x,y\in P$ then the following are equivalent: \begin{enumerate}
	\item $\leq y$
	\item $x^\downarrow\subseteq y^\downarrow$
	\item for all $Q\in \mathcal{O}(P)$ if $y\in Q$ then $x\in Q$
\end{enumerate}
\end{prop}
