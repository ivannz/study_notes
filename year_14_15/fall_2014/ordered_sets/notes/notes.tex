...
\selectlanguage{english}
\section{Galois connections} % (fold)
...
\end{description}
The maps $\phi$ and $\psi$ are \emph{dually adjoint} to each other.

\noindent\textbf{Theorem}\hfill \\ A pair of maps is a Galois connection if and only if for every $p\in P$ and $q\in Q$\begin{description}
...
\noindent\textbf{Theorem}\hfill \\If $(\phi,\psi)$ is a Galois connection, then $\phi = \phi\circ \phi\circ \phi$ and $\phi = \phi\circ \phi\circ \phi$.

Indeed, for any $p\in P$ \textbf{(CLOSURE)} implies that $p\leq \psi\brac{\phi(p)}$, whence by \textbf{(AMON)} $\phi{\psi\brac{\phi(p)}}\leq \phi(p)$.
However, \textbf{(CLOUSRE)} also implies that $\phi(p)\leq \phi\brac{\psi\brac{\phi(p)}}$. Similarly, for the second equality.

\noindent\textbf{Theorem}\hfill \\ A map $\phi:(P,\leq)\to (Q,\leq)$ between complete lattices has a dual adjoint if and only if for any $X\subseteq P$ it is true that \[\phi\brac{\sup X} = \inf \phi(X)\]

Suppose $\psi$ is dually adjoint to $\phi$. Since $y\leq \sup X$ for all $y\in X$ in $P$ be completeness of $P$, it must be true that $\phi\brac{\sup X}\leq \phi(y)$ by \textbf{(AMON)}, whence $\phi\brac{\sup X}\leq \inf_{y\in X} \phi(y) = \inf \phi(X)$. However, $\inf \phi(X)\leq \phi(y)$ for all $y\in X$ by completeness of $Q$, which implies by \textbf{(GALOIS)} that $y\leq \psi\brac{\inf \phi(X)}$ for all $y\in X$, whence $\sup X\leq \psi\brac{\inf \phi(X)}$. Therefore $\inf\phi(X)\leq \phi\brac{\sup X}$ by \textbf{(GALOIS)}.





% section galois_connections (end)

\selectlanguage{english}
\section{Concept lattices} % (fold)
\label{sec:concept_lattices}
...
\noindent\textbf{Theorem} \hfill \\ The derivation operators ${(\cdot)}'$ are a Galois connection.
...
\noindent\textbf{Theorem}\hfill\\ For any $\brac{A_j}_{j\in J}\subseteq G$ it is true that \[\brac{\bigcup_{j\in J}A_j}' = \bigcap_{j\in J} A_j'\]
...
If $A_j=A_j''$ for all $j\in J$, then \[\brac{\bigcap_{j\in J}A_j}'' = \brac{\bigcap_{j\in J}A_j''}'' = \brac{\bigcup_{j\in J}A_j'}''' = \brac{\bigcup_{j\in J}A_j'}' = \bigcap_{j\in J}A_j'' = \bigcap_{j\in J}A_j\]

\subsubsection{The main theorem of FCA} % (fold)
\label{ssub:the_main_theorem_of_fca}

\noindent\textbf{Theorem}\hfill \\ The set of all formal concepts of a context $(G,M,I)$ is a complete lattice $\cltc(G,M,I)$ with join and meet operations given by \[\bigvee_{j\in J} (A_j,B_j) = \brac{ \brac{\bigcup_{j\in J} A_j}'', \bigcap_{j\in J} B_j}\] and \[\bigwdge_{j\in J} (A_j,B_j) = \brac{\bigcap_{j\in J} A_j, \brac{\bigcup_{j\in J} B_j}''}\]
...
\noindent\textbf{Theorem}\hfill \\ For any context $(G,M,I)$ there exist maps $\gamma:G\to\cltc(G,M,I)$ and $\mu:M\to\cltc(G,M,I)$ such that $\gamma(G)$ and $\mu(M)$ are join- and meet-dense subsets of $\cltc(G,M,I)$. Furthermore, $(g,m)\in I$ if and only if $\gamma(g)\leq \mu(m)$ in $\cltc(G,M,I)$.
