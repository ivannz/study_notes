...
In order to find a complete and non-redundant subset of implications of some context first and foremost it is necessary to drop implications that hold in any context.
...
Let $A\subseteq M$ be some formal intent, i.e $A'' = A$. Any set $D\subseteq A$ with $D'' = A''$ is a \emph{generator} of $A$. The collection of all generators of $A$ is \[\text{gen}(A) \defn \obj{\induc{D\subseteq A}\, D'' = A'' }\]
...
Consider a context $(G,M,I)$. A subset $P\subseteq M$ is a pseudo-intent if $P\neq P''$ and $Q''\subseteq P$ for every pseudo-intent $Q\subseteq P$ and $Q\neq P$. \textbf{Duquenne–Guigues basis}, or the \textbf{stem base}, of attribute implications is defined as \[\text{DQbas}(G,M,I)\defn \obj{\induc{ P\to \brac{P''\setminus P} }\, P \text{-- pseudo-intent} }\]

What was the rationale behind the notion of pseudo-intents?

\begin{prop} The stem base is non-redundant and complete.
\end{prop}

\begin{proof}\hfill
\begin{description}
\item[Inclusion] \hfill \\ For any $P\to P''\in \Kcal$ it is true that $P''\subseteq \brac{P}''$ whence $\Kcal$ holds in $(G,M,I)$.

\item[Completeness] \hfill \\ Suppose $T\subseteq M$ is such that $T\neq T''$. If $T$ is a pseudo-intent, then $T\to T''$ is an implication from $\Kcal$ which is not respected by $T$ because $T''\not\subseteq T$ violates the requirement that either $T\not\subseteq T$ or $T''\subseteq T$.

If $T$ is not a pseudo-intent, then there exists a pseudo-intent $Q\subseteq T$ such that $Q''\not\subseteq T$, whence an implication $Q\to Q''$ from $\Kcal$ cannot be respected by $T$. Therefore every subset $T\subseteq M$ with $T\neq T''$ cannot be a model of $\Kcal$.
% Suppose $T\subseteq M$ is a model of $\Kcal$, but $T\neq T''$. The set $T$ cannot be a pseudo-intent, because in that case the implication $T\to T''$ would be have to be respected by $T$, which would imply that $T''\subseteq T$ and further $T'' = T$.
% If there is no pseudo-intent $P$ with $P\subset T$ then $T$ is itself a pseudo-intent. However, if $P\subset T$ for some pseudo-intent $P$, then $T$, being a model of $\Kcal$, must respect the implication $P\to P''$, whence $P''\subseteq T$, implying that $T$ is a pseudo-intent.
...
However, being a pseudo-intent, $Q$ cannot respect the implication $Q\to Q''$ because $Q''\neq Q$. Therefore \[\Kcal\setminus \obj{Q\to Q''}\not\models Q\to Q''\] Thus omitting any implication from $\Kcal$ results in an incomplete set of implications.
\end{description}
\end{proof}
...
\subsubsection{JSM on a binary context} % (fold)
\label{ssub:dsm_on_a_binary_context}
...
% subsubsection dsm_on_a_binary_context (end)

\subsubsection{JSM on non-binary scale} % (fold)
\label{ssub:dsm_on_non_binary_scale}
% 4.2 ДСМ на небинарном со шкалированием

% subsubsection dsm_on_non_binary_scale (end)

\subsubsection{JSM on patterns structures} % (fold)
\label{ssub:dsm_on_patterns_structures}
% 4.3 ДСМ на узорных структурах

% subsubsection dsm_on_patterns_structures (end)

\subsubsection{Application to graphs} % (fold)
\label{ssub:application_to_graphs}
% 4.4 пример на графах (вроде точно будет в контрольной - Кузнецов говорил!)

% subsubsection application_to_graphs (end)
