...
= \sum_{i=1}^N \sum_{k=1}^K \hat{u}_k(i) \log \pi_k + \sum_{i=1}^N \sum_{k=1}^K \hat{u}_k(i)\log \phi(x_i\lvert \m\hat{u}_k, \Sigma_k)
\]
...
\[\sum_{i=1}^N \sum_{k=1}^K \hat{u}_k(i)\log \phi(x_i\lvert \m\hat{u}_k, \Sigma_k)\]
...
\]
Since $\Sigma_k$ is a symmetric matrix, this expression is equivalent to
\[
