...
\[g'(x) = c_k + \frac{1}{2}\frac{\gamma_{k+1}(x-\xi_k)^2-\gamma_k(x-\xi_{k+1})^2}{\xi_{k+1}-\xi_k}\]
(?)...
%% See 20150405_184027.jpg
Finally the NCS is given by:
\[
g(x) = \begin{cases}
	g_1 - (\xi_1-x)g'(\xi_1), &\text{ if } x\leq \xi_1\\
	g_K - (x-\xi_K)g'(\xi_K), &\text{ if } x\geq \xi_K\\
	% \frac{(x-\xi_k)g_{k+1} + (\xi_k-x)g_k}{h_k}
	\frac{1}{h_k}\bigl(g_{k+1}(x-\xi_k) + g_k(\xi_{k+1}-x)\bigr)
		- \frac{1}{6}(x-\xi_k)(\xi_{k+1}-x) \cdot \\
		\quad\cdot\Bigl\{ g_{k+1}\bigl( 1+\frac{x-\xi_k}{h_k}\bigr)
			+ g_k\bigl( 1+\frac{\xi_{k+1}-x}{h_k}\bigr) \Bigr\}, &\text{otherwise}
\end{cases}
\]
where $h_k = \xi_{k+1}-\xi_k$.
