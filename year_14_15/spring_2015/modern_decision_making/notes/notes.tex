...
= \langle \sum_{i=1}^n \lambda_i \Phi(x_i), \sum_{j=1}^n \lambda_j \Phi(x_j)\rangle_\Hcal
...
%% Proof:
Let $C$ be a convex subset of $\Hcal$, i.e. for any $x,y\in C$ and all $t\in[0,1]$
one has $t x + (1-t) y \in C$. Let $x_0\in \Hcal$ be arbitrary.
...
Now, by continuity at $x-0$ there exists $\eta > 0$ such that for all $x\in \Hcal$
with $\|x_0-x\| \geq \eta$ :
