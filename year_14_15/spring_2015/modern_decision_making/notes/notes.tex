...
Sample space is augmented and now has $M$ predictors. The regression function becomes:
...
In general $M$-\textbf{splines} are piecewise polynomial regression curves with $M$-
smoothness restriction at binding knots:
\begin{align*}
	h_j(x) &= x^{j-1}&\text{for }j=1,\ldots, M\\
	h_{M+j}(x) &= (x-\xi_j)_+^M &\text{for }j=1,\ldots,K
\end{align*}

One polynomial -- global and smooth. More polynomials with a local regression are
not necessarily smooth. Splines are local and smooth!
...
Within the range of the univariate data fit a spline, but to achieve robustness
at the ends and get more trustworthy predictions fit a linear model!
