...
%%% HERE!!! Solve the third tutorial and write it here!!
...
\begin{itemize}
	...
	\item the eigenvalues of $K$ do not depend on $\lambda$.
\end{itemize}
...
The solution to the weighted problem
\[
\sum_{i=1}^n \omega_i \bigl(t_i - g(x_i)\bigr)^2
+ \lambda \int |g''|^2 ds
\to \min_{g\in C^2[a,b]}
\]
is again in the Natural Cubic spline family, since we can consider an NCS with
the same goodness-of-fit term, but with apriori smaller penalty term (recall the
proof that the natural cubic spline has the least penalty in $C^[a,b]$). The solution
in the weighted case is
