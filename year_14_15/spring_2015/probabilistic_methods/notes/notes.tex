...
% Конференция по современным методам оптимизации, посвящённая 80летию Б. Т. Поляка.
No lecture on the $14$-th of May. \url{http://sites.google.com/site/polconf}

\subsection{Brownian motion (cntd)} % (fold)
...
% Kingman Poisson processes

\subsubsection{Fokker-Planck equation} % (fold)
...
% subsubsection fokker_planck_equation (end)

\subsubsection{Solution of the equation without drift} % (fold)
...
% subsubsection solution_of_the_equation_without_drift (end)

% subsection brownian_motion_cntd (end)
