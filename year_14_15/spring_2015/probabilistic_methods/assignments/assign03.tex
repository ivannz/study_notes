...
Совметсная функция плотности $(X,D)$ может быть разложена в следующее произведение
...
Пусть случайная величина $Z = \frac{\lambda^2}{D}$. Тогжа её плотность равна
...
\]
Таким образом $Z$ имеет распределение $\Gamma\bigl(\frac{1}{2},\frac{1}{2}\bigr)$ котрое на
самом деле есть $\chi^2_1$.

Учитывая данное наблдение модель итоговой случайной величины принимает вид:
\[X\rvert Z\sim \Ncal\bigl(0,\frac{\lambda^2}{Z}\bigr)\text{ где } Z\sim \Gamma\bigl(\frac{1}{2},\frac{1}{2}\bigr)\]
Замтеим, что условная плотность $X$ при заданном $Z=z$ равна
\[p_{X\rvert Z}(x\rvert z) = \frac{z^\frac{1}{2}}{\sqrt{ 2\pi \lambda^2}} e^{-\frac{x^2}{2\lambda^2}z}\]
Тогда искомая беузсловная плотность $X$ равна
\[
p_X(x)
...
\]
Интерал упрощается следующим образом:
\begin{align*}
	p_X(x)
	&= \int_0^\infty \frac{z^\frac{1}{2}}{\sqrt{ 2\pi \lambda^2}} e^{-\frac{x^2}{2\lambda^2}z}
		\frac{1}{\sqrt{2\pi}} z^{\frac{1}{2}-1} e^{-\frac{1}{2}z} dz \\
	...
	&=\Bigl[ \beta = \frac{1}{2} \bigl(\frac{x^2}{\lambda^2}+1\bigr) \Bigr]\\
...
\]
Фнукция распределения данной случайной величины задаётся
\[
P_X(x)
...
\]
Таким образом, случайная величина $X$ распределена согласно распределнию Коши
